...
class GroupPolicyScaleInAction(BaseModel):
    """
    ...
    """ # noqa: E501
    amount: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="'amountType=ABSOLUTE' specifies the absolute number of VMs that are added or removed. The value must be between 1 to 10.   'amountType=PERCENTAGE' specifies the percentage value that is applied to the current number of replicas of the VM Auto Scaling Group. The value must be between 1 to 200.   At least one VM is always added or removed.   Note that for 'SCALE_IN' operations, volumes are not deleted after the server is deleted.")
    amount_type: Optional[ActionAmount] = Field(default=None, alias="amountType")
    cooldown_period: Optional[StrictStr] = Field(default=None, description="The minimum time that elapses after the start of this scaling action until the next scaling action is started. With a scaling action in progress, no second scaling action is started for the same VM Auto Scaling Group. Instead, the metric is re-evaluated after the current scaling action completes (either successfully or with errors). This is currently validated with a minimum value of 2 minutes and a maximum value of 24 hours. The default value is 5 minutes if not specified.", alias="cooldownPeriod")
    termination_policy: Optional[TerminationPolicyType] = Field(default=None, alias="terminationPolicy")
    delete_volumes: Optional[StrictBool] = Field(default=None, description="If set to `true`, when deleting an replica during scale in, any attached volume will also be deleted. When set to `false`, all volumes remain in the datacenter and must be deleted manually.  **Note**, that every scale-out creates new volumes. When they are not deleted, they will eventually use all of your contracts resource limits. At this point, scaling out would not be possible anymore.", alias="deleteVolumes")
    __properties: ClassVar[List[str]] = ["amount", "amountType", "cooldownPeriod", "terminationPolicy", "deleteVolumes"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "terminationPolicy": obj.get("terminationPolicy"),
            "deleteVolumes": obj.get("deleteVolumes")
        })
        return _obj
